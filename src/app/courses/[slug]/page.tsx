...
export async function generateMetadata({ params }: CoursePageProps): Promise<Metadata> {
  ...
  const course = COURSE_MODULES.find((item) => item.slug === resolvedSlug);
  ...
  return {
    title: `${course.title} | IgniteCore Courses`,
...
export default async function CourseDetailPage({ params }: CoursePageProps) {
  ...
  const articleSchema = {
    ...
    teaches: course.keyword,
  };

  return (
    <section className="section-shell">
      <Script id={`course-schema-${course.slug}`} type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(articleSchema) }} />
      <article className="mx-auto w-full max-w-[900px] px-4 md:px-6">
        <p className="font-mono text-xs uppercase tracking-[0.16em] text-[var(--color-teal)]">
          Course lesson · {course.category} · {course.duration}
        </p>
        <h1 className="mt-3 font-display text-4xl md:text-5xl">{course.title}</h1>
        <p className="mt-3 text-sm text-[var(--color-slate)]">{course.summary}</p>

        <div className="mt-6 overflow-hidden rounded-xl border border-[var(--color-border)] bg-white p-2">
          <iframe src={course.videoEmbed} title={`${course.title} video`} className="h-[420px] w-full rounded-lg" loading="lazy" />
        </div>
        ...
        <div className="mt-8 grid gap-4 md:grid-cols-2">
          <article className="surface-card p-6">
            ...
            <ul className="mt-4 space-y-2 text-sm text-[var(--color-deep-navy)]">
              ...
              <li>Modules: {course.lessons} key points</li>
          ...
          <article className="surface-card p-6">
            ...
            <div className="mt-4 flex flex-col gap-3">
              <a href={course.downloadPath} download className="rounded-md border border-[var(--color-border)] px-4 py-3 text-center text-sm font-semibold hover:bg-[var(--color-cream)]">
                Download lesson notes
              </a>
              <a href={course.publicResource} target="_blank" rel="noreferrer" className="rounded-md border border-[var(--color-border)] px-4 py-3 text-center text-sm font-semibold hover:bg-[var(--color-cream)]">
